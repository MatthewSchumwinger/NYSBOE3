...
results in:  
 ...
 - were from a contributor that had given to the th recipient committee not more than twice ever. (NOTE: this is broader than looking just in the time preceeding the contribution.)
...
# df <- trans1
df <- setDT(tBigRare) #TODO: need to set ID in first step to make this work
...
justOuts <- df %>% filter(direction == "out") #TODO: need to connect this to trans1
...
flagged <- ans %>% filter(!is.na(direction)) %>%
  # group_by(i.transID, FILER_NAME, TRANSACTION_CODE) %>% # this is grouped by outgoing transaction type
  group_by(i.transID, FILER_NAME) %>%
  summarise(
            nOutTrans = n(),
...
flagged <- flagged %>% select(-c(FILER_NAME.y, direction, end_30))
names(flagged) <- c("transID","FILER_NAME", "DATE1_10","IN_TRANSACTION_CODE",
                    "AMOUNT_70","nOutTrans","totalOut" )

end <- Sys.time()
end - start
View(flagged)
table(flagged$transID)
